...
import { Star, Wine, Beer, Utensils, ThumbsUp, Share2, Crown, Heart } from 'lucide-react';
...
export default function PairingDetails() {
  ...
  useEffect(() => {
    // Add to history when pairing is loaded
    if (user && pairing && !id?.startsWith('mock-')) {
      addToHistory(pairing.id);
    }
  }, [user, pairing]);
  ...
  const addToHistory = async (pairingId: string) => {
    try {
